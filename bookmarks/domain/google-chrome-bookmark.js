import { DOMParser } from "@xmldom/xmldom";

export function getBookmarkAnchorTags(content) {
  const bookmarkAnchorTags = [];
  for (const line of content) {
    if (line.trimStart().startsWith("<DT><A HREF=\"https://www.amazon.co.jp/")) {
      bookmarkAnchorTags.push(line);
    }
  }
  return bookmarkAnchorTags
}

// 以下の書式を与えるとtitleとurlを返す
// <DT><A HREF="https://www.amazon.co.jp/dp/4048931164?tag=kadoofce-22" ADD_DATE="1716536377" ICON="data:image/png;base64,">Amazon.co.jp: 関数型ドメインモデリング ドメイン駆動設計とF#でソフトウェアの複雑さ…</A>
// Bookmark HTMLの<DT>は閉じタグがないので、入力文字列から削除する仕様である
export function parseAnchorTag(content) {
  // <DT> および </DT> を削除
  const anchorTag = content.replace(/<\/?DT>/g, '');
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(anchorTag, "text/xml");
  const anchorElement = xmlDoc.getElementsByTagName("A")[0];
  const url = anchorElement.getAttribute("HREF");
  const title = anchorElement.textContent;
  return { title: title, url: url };
}